...
for k=1:5
   for i=1:La
     ...
     s=j*u;
       ...
       lambda1ew=(vew(k)+sqrt(vew(k)^2+4*D*s))/(2*D);
       lambda2ew=(vew(k)-sqrt(vew(k)^2+4*D*s))/(2*D);
       %equation（64）
...
% Double logarithmic curves of concentration of pollutant at the wellhead in real-space at different vew
loglog(t,ft(:,1),t,ft(:,2),t,ft(:,3),t,ft(:,4),t,ft(:,5), 'k-','Linewidth',1.)
hold on 
axis([10^(0) 10^(4) 580 10^(4.7)]);

xlabel({'$$t (h)$$ ','','Figure 12 Concentration curves of the effect of $$v_{ew}$$'},'FontName','Times New Roman','FontSize',10,'Interpreter','latex') 
ylabel('$$\mu_{ew} (mg/L)$$' ,'FontName','Times New Roman','FontSize',10,'Interpreter','latex')
legend('$$ v_{ew}=3 $$','$$ v_{ew}=3.6 $$','$$ v_{ew}=4.2 $$','$$ v_{ew}=4.8 $$','$$ v_{ew}=5.4 $$','Interpreter','latex')
grid on  
