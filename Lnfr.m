...
for k=1:5
   for i=1:La
     ...
     s=j*u;
       ...
       %equation（66）
       znfr=(1-1/(2*m))*Lnfr(k);%(17)
       gnfr=cnfr*xnf./L+dnfr*znfr/Lnfr(k)+wnfr;
...
% Double logarithmic curves of concentration of pollutant at the wellhead in real-space at different Lnfr
loglog(t,ft(:,1),t,ft(:,2),t,ft(:,3),t,ft(:,4),t,ft(:,5), 'k-','Linewidth',1.)
hold on 
axis([10^(0) 10^(4) 580 10^(4.7)]);

xlabel({'$$t (h)$$ ','','Figure 14 Concentration curves of the effect of $$L_{nfr}$$'},'FontName','Times New Roman','FontSize',10,'Interpreter','latex') 
ylabel('$$\mu_{ew} (mg/L)$$' ,'FontName','Times New Roman','FontSize',10,'Interpreter','latex')
legend('$$ L_{nfr}=0.2 $$','$$ L_{nfr}=0.5 $$','$$ L_{nfr}=0.8 $$','$$ L_{nfr}=1.1 $$','$$ L_{nfr}=1.4 $$','Interpreter','latex')
grid on  
