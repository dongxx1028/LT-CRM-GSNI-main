...
wnfl=1;
wnfr=1;
D =[4.5*10^(-4),5.0*10^(-4),5.5*10^(-4),6*10^(-4),6.5*10^(-4)];
xnf=0.01;
z=0;
...
for k=1:5
   for i=1:La
     ...
     s=j*u;
       ...
       lambda1nfl=(vnfl+sqrt(vnfl^2+4*D(k)*(s+zetanfl)))/(2*D(k));
       lambda2nfl=(vnfl-sqrt(vnfl^2+4*D(k)*(s+zetanfl)))/(2*D(k));
       %The characteristic root parameters of the left wing of the first or mth crack
       lambda1nfla=(vnfl+sqrt(vnfl^2+4*D(k)*(s+zetanfl-anfl/eata)))/(2*D(k));
       lambda2nfla=(vnfl-sqrt(vnfl^2+4*D(k)*(s+zetanfl-anfl/eata)))/(2*D(k));
       %The characteristic roots of the right wing of the first or the n-th crack,equation（36）-（37） 
       lambda1nfr=(vnfr+sqrt(vnfr^2+4*D(k)*(s+zetanfr)))/(2*D(k));
       lambda2nfr=(vnfr-sqrt(vnfr^2+4*D(k)*(s+zetanfr)))/(2*D(k));
       %Characteristic root parameters of the right-wing features of the first or n cracks-anfl/eata，
       lambda1nfra=(vnfr+sqrt(vnfr^2+4*D(k)*(s+zetanfr-anfr/eata)))/(2*D(k));
       lambda2nfra=(vnfr-sqrt(vnfr^2+4*D(k)*(s+zetanfr-anfr/eata)))/(2*D(k));
       %The characteristic root from the first or the n-th crack to the bottom end of the vertical wellbore,equation（55）-（56） 
       lambda1ne=(vne+sqrt(vne^2+4*D(k)*s))/(2*D(k));
       lambda2ne=(vne-sqrt(vne^2+4*D(k)*s))/(2*D(k));
       %The characteristic root from the bottom of the vertical wellbore to the wellhead,quation（60）-（61） 
       lambda1ew=(vew+sqrt(vew^2+4*D(k)*s))/(2*D(k));
       lambda2ew=(vew-sqrt(vew^2+4*D(k)*s))/(2*D(k));
       %equation（64）
...
% Double logarithmic curves of concentration of pollutant at the wellhead in real-space at different D
loglog(t,ft(:,1),t,ft(:,2),t,ft(:,3),t,ft(:,4),t,ft(:,5), 'k-','Linewidth',1.)
hold on 
axis([10^(0) 10^(4) 580 10^(4.7)]);

xlabel({'$$t (h)$$ ','','Figure 17 Concentration curves of the effect of $$D$$'},'FontName','Times New Roman','FontSize',10,'Interpreter','latex') 
ylabel('$$\mu_{ew} (mg/L)$$' ,'FontName','Times New Roman','FontSize',10,'Interpreter','latex')
legend('$$ D=4.5\times10^{-4} $$','$$ D=5.0\times10^{-4} $$','$$ D=5.5\times10^{-4} $$','$$ D=6.0\times10^{-4} $$','$$ D=6.5\times10^{-4} $$','Interpreter','latex')
grid on  
