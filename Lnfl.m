...
for k=1:5
   for i=1:La
     ...
     s=j*u;
       ...
       %equation（64）
       znfl=-(1-1/(2*m))*Lnfl(k);%(16)
       gnfl=cnfl*xnf./L+dnfl*znfl/Lnfl(k)+wnfl;
       %equation（66）
...
% Double logarithmic curves of concentration of pollutant at the wellhead in real-space at different Lnfl
loglog(t,ft(:,1),t,ft(:,2),t,ft(:,3),t,ft(:,4),t,ft(:,5), 'k-','Linewidth',1.)
hold on 
axis([10^(0) 10^(4) 580 10^(4.7)]);

xlabel({'$$t (h)$$ ','','Figure 13 Concentration curves of the effect of $$L_{nfl}$$'},'FontName','Times New Roman','FontSize',10,'Interpreter','latex') 
ylabel('$$\mu_{ew} (mg/L)$$' ,'FontName','Times New Roman','FontSize',10,'Interpreter','latex')
legend('$$ L_{nfl}=0.1 $$','$$ L_{nfl}=0.2 $$','$$ L_{nfl}=0.3 $$','$$ L_{nfl}=0.4 $$','$$ L_{nfl}=0.5 $$','Interpreter','latex')
grid on  
