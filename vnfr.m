...
for k=1:5
   for i=1:La
     ...
     s=j*u;
       ...
       lambda1nfr=(vnfr(k)+sqrt(vnfr(k)^2+4*D*(s+zetanfr)))/(2*D);
       lambda2nfr=(vnfr(k)-sqrt(vnfr(k)^2+4*D*(s+zetanfr)))/(2*D);
       %Characteristic root parameters of the right-wing features of the first or n cracks-anfl/eata，
       lambda1nfra=(vnfr(k)+sqrt(vnfr(k)^2+4*D*(s+zetanfr-anfr/eata)))/(2*D);
       lambda2nfra=(vnfr(k)-sqrt(vnfr(k)^2+4*D*(s+zetanfr-anfr/eata)))/(2*D);
...
% Double logarithmic curves of concentration of pollutant at the wellhead in real-space at different vnfr
loglog(t,ft(:,1),t,ft(:,2),t,ft(:,3),t,ft(:,4),t,ft(:,5), 'k-','Linewidth',1.)
hold on 
axis([10^(0) 10^(4) 580 10^(4.7)]);

xlabel({'$$t (h)$$ ','','Figure 10 Concentration curves of the effect of $$v_{nfr}$$'},'FontName','Times New Roman','FontSize',10,'Interpreter','latex') 
ylabel('$$\mu_{ew} (mg/L)$$' ,'FontName','Times New Roman','FontSize',10,'Interpreter','latex')
legend('$$ v_{nfr}=3.6 $$','$$ v_{nfr}=4.8 $$','$$ v_{nfr}=6 $$','$$ v_{nfr}=7.2 $$','$$ v_{nfr}=8.4 $$','Interpreter','latex')
grid on  
