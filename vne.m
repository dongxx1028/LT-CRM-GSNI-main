...
for k=1:5
   for i=1:La
     ...
     s=j*u;
       ...
       lambda1ne=(vne(k)+sqrt(vne(k)^2+4*D*s))/(2*D);
       lambda2ne=(vne(k)-sqrt(vne(k)^2+4*D*s))/(2*D);
...
% Double logarithmic curves of concentration of pollutant at the wellhead in real-space at different vne
loglog(t,ft(:,1),t,ft(:,2),t,ft(:,3),t,ft(:,4),t,ft(:,5), 'k-','Linewidth',1.)
hold on 
axis([10^(0) 10^(4) 580 10^(4.7)]);

xlabel({'$$t (h)$$ ','','Figure 11 Concentration curves of the effect of $$v_{ne}$$'},'FontName','Times New Roman','FontSize',10,'Interpreter','latex') 
ylabel('$$\mu_{ew} (mg/L)$$' ,'FontName','Times New Roman','FontSize',12,'Interpreter','latex')
legend('$$ v_{ne}=1.2 $$','$$ v_{ne}=2.4 $$','$$ v_{ne}=3.6 $$','$$ v_{ne}=4.8 $$','$$ v_{ne}=5.0 $$','Interpreter','latex')
grid on  
