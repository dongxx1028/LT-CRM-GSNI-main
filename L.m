...
for k=1:5
   for i=1:La
     ...
     s=j*u;
       ...
       lambda1nfl=(vnfl+sqrt(vnfl^2+4*D*(s+zetanfl)))/(2*D);
       lambda2nfl=(vnfl-sqrt(vnfl^2+4*D*(s+zetanfl)))/(2*D);
       %The characteristic root parameters of the left wing of the first or mth crack
       lambda1nfla=(vnfl+sqrt(vnfl^2+4*D*(s+zetanfl-anfl/eata)))/(2*D);
       lambda2nfla=(vnfl-sqrt(vnfl^2+4*D*(s+zetanfl-anfl/eata)))/(2*D);
       ...
       gnfl=cnfl*xnf./L(k)+dnfl*znfl/Lnfl+wnfl;
       %equation（66）
       znfr=(1-1/(2*m))*Lnfr;%(17)
       gnfr=cnfr*xnf./L(k)+dnfr*znfr/Lnfr+wnfr;
       ...
       Cne=exp(lambda2ne*L(k))*((Cnfl+ Cnfr)/2+(An+Bn*lambda1ne)/((lambda2ne-lambda1ne)*lambda1ne^2)+(An+Bn*lambda2ne)/((lambda1ne-lambda2ne)*lambda2ne^2))...
           -(An*lambda1ne*L(k)+An+Bn*lambda1ne)/((lambda2ne-lambda1ne)*lambda1ne^2)-(An*lambda2ne*L(k)+An+Bn*lambda2ne)/((lambda1ne-lambda2ne)*lambda2ne^2);
       ...
       Cew=exp(lambda2ew*H)*(Cne+(Aw+Bw*lambda1ew)/((lambda2ew-lambda1ew)*lambda1ew^2)+(Aw+Bw*lambda2ew)/((lambda1ew-lambda2ew)*lambda2ew^2))...
           -(Aw*lambda1ew*H+Aw+Bw*lambda1ew)/((lambda2ew-lambda1ew)*lambda1ew^2)-(Aw*lambda2ew*L(k)+Aw+Bw*lambda2ew)/((lambda1ew-lambda2ew)*lambda2ew^2);
...
% Double logarithmic curves of concentration of pollutant at the wellhead in real-space at different L
loglog(t,ft(:,1),t,ft(:,2),t,ft(:,3),t,ft(:,4),t,ft(:,5), 'k-','Linewidth',1.)
hold on 
axis([10^(0) 10^(4) 580 10^(4.7)]);

xlabel({'$$t (h)$$ ','','Figure 15 Concentration curves of the effect of $$L$$'},'FontName','Times New Roman','FontSize',10,'Interpreter','latex') 
ylabel('$$\mu_{ew} (mg/L)$$' ,'FontName','Times New Roman','FontSize',10,'Interpreter','latex')
legend('$$ L=0.6 $$','$$ L=0.8 $$','$$ L=1 $$','$$ L=1.2 $$','$$ L=1.4 $$','Interpreter','latex')
grid on  
