...
for k=1:5
   for i=1:La
     ...
     s=j*u;
       ...
       Cew=exp(lambda2ew*H(k))*(Cne+(Aw+Bw*lambda1ew)/((lambda2ew-lambda1ew)*lambda1ew^2)+(Aw+Bw*lambda2ew)/((lambda1ew-lambda2ew)*lambda2ew^2))...
           -(Aw*lambda1ew*H(k)+Aw+Bw*lambda1ew)/((lambda2ew-lambda1ew)*lambda1ew^2)-(Aw*lambda2ew*L+Aw+Bw*lambda2ew)/((lambda1ew-lambda2ew)*lambda2ew^2);
...
% Double logarithmic curves of concentration of pollutant at the wellhead in real-space at different H
loglog(t,ft(:,1),t,ft(:,2),t,ft(:,3),t,ft(:,4),t,ft(:,5), 'k-','Linewidth',1.)
hold on 
axis([10^(0) 10^(4) 580 10^(4.7)]);

xlabel({'$$t (h)$$ ','','Figure 16 Concentration curves of the effect of $$H$$'},'FontName','Times New Roman','FontSize',10,'Interpreter','latex') 
ylabel('$$\mu_{ew} (mg/L)$$' ,'FontName','Times New Roman','FontSize',10,'Interpreter','latex')
legend('$$ H=9 $$','$$ H=12 $$','$$ H=15 $$','$$ H=18 $$','$$ H=21 $$','Interpreter','latex')
grid on  
